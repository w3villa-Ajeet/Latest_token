...
!developmentChains.includes(network.name)
  ? describe.skip
  : describe("LatestTokenLock", async () => {
      ...
      beforeEach(async () => {
        ...
        lockToken = await ethers.getContract("LatestTokenLock", owner)
      })

      describe("LatestTokenLock", async () => {
        it("Should not transfer tokens to own address", async () => {
          const amount = ethers.BigNumber.from("100")
          await expect(lockToken.lockTokens(amount, owner)).to.be.revertedWith(
            "CAN_NOT_TRANSFER_TOKEN_TO_OWN"
          )
        })
        ...
        it("should not Allow User to withdraw Tokens Before Time", async () => {
          ...
          await expect(
            lockToken.connect(signer).claimTokens()
          ).to.be.revertedWith("TOKENS_ARE_STILL_LOCKED")
        })
        ...
        it("Should Not be allow user to withdraw tokens again ", async () => {
          ...
          await expect(
            lockToken.connect(signer).claimTokens()
          ).to.be.revertedWith("BENEFICIARY_NOT_FOUND")
        })
      })
    })
